...
def ratings(
    ...
    template_name = (
        template_name
        or context.get("star_ratings_template_name")
        or "star_ratings/widget.html"
    )
    return loader.get_template(template_name).render(
        {
            ...
            * (rating.average / Decimal(app_settings.STAR_RATINGS_RANGE)),
            ...
            and (is_authenticated(request.user) or app_settings.STAR_RATINGS_ANONYMOUS),
            "clearable": not read_only
            and (
                is_authenticated(request.user) and app_settings.STAR_RATINGS_CLEARABLE
            ),
        },
        request=request,
    )
