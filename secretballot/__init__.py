...
def limit_total_votes(num):
    ...
    def total_vote_limiter(request, content_type, object_id, vote):
        return (
            Vote.objects.filter(
                content_type=content_type, token=request.secretballot_token
            ).count()
            < num
        )
...
def enable_voting_on(
    ...
    class VotableManager(base_manager):
        ...
        def get_queryset(self):
            ...
            content_type_id_query = (
                "(SELECT id FROM {} WHERE app_label='{}' AND model='{}')".format(
    ...
    cls._meta.local_managers[:] = (
        manager
        for manager in cls._meta.local_managers
        if not manager.name == manager_name
    )
